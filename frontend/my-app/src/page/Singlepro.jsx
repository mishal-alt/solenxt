...
function Singleproduct() {
  ...
  return (
    <div className="bg-[#050505] min-h-screen text-gray-200 font-sans selection:bg-cyan-500 selection:text-black pt-24 md:pt-40 pb-20">
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-4 mb-4">
                <span className="bg-gradient-to-r from-cyan-500 to-blue-600 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider">
                  Limited Edition
                </span>
              ...
              <p className="mt-4 text-gray-400 text-sm md:text-lg leading-relaxed max-w-md italic">
            ...
            <div className="flex items-center gap-6">
              ...
              <span className="text-[10px] text-green-400 font-black bg-green-500/10 px-3 py-1 rounded-lg border border-green-500/20 tracking-widest">
                20% OFF
              </span>
            </div>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {['7', '8', '9', '10', '11'].map((size) => (
                    <div
                      key={size}
                      className="w-10 h-10 rounded-lg text-xs font-bold flex items-center justify-center border bg-white/5 border-white/10 text-gray-300"
                    >
                      {size}
                    </div>
                  ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              {/* Quantity */}
              <div className="flex items-center justify-between sm:justify-start bg-white/5 rounded-xl border border-white/10 px-4 h-14 w-full sm:w-fit">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="text-gray-400 hover:text-white p-2">
                  <Minus className="w-4 h-4" />
                </button>
                <span className="w-12 text-center font-bold text-white">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="text-gray-400 hover:text-white p-2">
              ...
              <button
                ...
                className={`flex-1 h-14 rounded-xl font-black uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-2 transition-all shadow-lg ${product.stoke <= 0
            ...
            <div className="grid grid-cols-3 gap-2 md:gap-4 pt-8">
              {[
                ...
                <div key={i} className="flex flex-col items-center justify-center p-3 md:p-4 rounded-xl bg-white/5 border border-white/5 text-center gap-2 group hover:bg-white/10 transition-colors">
          ...
          >
            {/* Main Image Card */}
            <div className="relative z-10 rounded-[2rem] md:rounded-[3rem] bg-gradient-to-br from-gray-800/20 to-black border border-white/10 p-6 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-sm group">
              <div className="absolute top-0 right-0 w-32 md:w-64 h-32 md:h-64 bg-cyan-500/10 rounded-full blur-[60px] md:blur-[100px] -z-10 pointer-events-none" />

              <motion.img
                src={product.image.startsWith("/") ? `${BASE_URL}${product.image}` : product.image}
                alt={product.name}
                className="w-full h-auto object-contain drop-shadow-[0_15px_15px_rgba(0,0,0,0.5)] md:drop-shadow-[0_25px_25px_rgba(0,0,0,0.5)]"
              />

              {/* Floating Stock Card */}
              <div className="absolute -bottom-4 md:-bottom-6 -left-2 md:-left-6 bg-black/90 backdrop-blur-md border border-white/10 p-3 md:p-4 rounded-xl md:rounded-2xl shadow-xl flex items-center gap-3 md:gap-4">
                ...
                <div>
                  <p className="text-[8px] md:text-xs text-gray-400 font-black uppercase tracking-widest">Stock</p>
                  <p className="text-xs md:text-sm font-black text-white">{product.stoke} Left</p>
                </div>
              </div>
            </div>

            {/* Gallery Thumbnails */}
            <div className="flex justify-center gap-3 mt-8 md:mt-12 overflow-x-auto pb-2 scrollbar-hide">
              {galleryImages.map((img, index) => (
                <div key={index} className="w-16 md:w-20 h-16 md:h-20 flex-shrink-0 rounded-xl border border-white/10 bg-white/5 p-2 cursor-pointer hover:border-cyan-500 transition-colors overflow-hidden">
